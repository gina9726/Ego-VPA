...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompt_cfg={}, prompt_learner=None, prompt_generator=None):
        ...
        self.use_basis = prompt_cfg.pop('use_basis', False)
        if self.num_tokens > 0:
            ...
            if self.prompt_generator is not None:
                if self.prompt_generator.use_basis:
                    self.k_s = len(self.prompt_generator.prompt_basis)
    ...
    def forward(self, x: torch.Tensor, pos_emb, use_checkpoint=False, istrain=False, gamma=1.0, eot=None):
        ...
        for i, blk in enumerate(self.resblocks):
            if self.num_tokens > 0 and self.use_basis:
                k = self.num_tokens
                ...
                if i < self.k_s:
                    out = self.prompt_generator.prompt_basis[i](query, k, istrain=istrain, gamma=gamma)
